...
export default function CourseManagementPage() {
  const { channelId } = useParams({
    from: "/channel/$channelId/courses/manage",
  });
  const { identity } = useAuth();
  const { data: channel, isLoading } = useGetChannel(channelId);
  const { data: courses = [], isLoading: coursesLoading } =
    useGetChannelCourses(channelId);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Create and manage video courses for your channel
              </CardDescription>
            </div>
        ...
        <CardContent>
          {coursesLoading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">
              No courses yet. Create your first course to organize your videos
              into structured learning paths!
            </p>
          ) : (
            <div className="space-y-4">
              {courses.map((course) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      {course.priceUsd && (
                        <span>${Number(course.priceUsd)}</span>
                      )}
                      {course.requiredTierLevel && (
                        <span>Tier {Number(course.requiredTierLevel)}+</span>
                      )}
                      <span
                        className={
                          course.isVisible
